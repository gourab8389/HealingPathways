  ...
  return (
    <div className='mb-[25px] flex flex-col-reverse justify-between gap-6 md:flex-row'>
      ...
      <div className="relative flex items-center justify-between sm:hidden">
        ...
        <img src={menu} alt="menu" className='h-[34px] w-[34px] cursor-pointer object-contain' onClick={()=>{setToggleDrawer((prev)=> !prev)}}/>

        <div className={`absolute left-0 right-0 top-[60px] px-2 z-10 bg-[#1c1c24] py-4 shadow-secondary ${!toggleDrawer ? "-translate-y-[100vh]":"translate-y-0"} transition-all duration-700`}>
            <ul className='mb-4'>
                {navLinks.map((link)=> (
                    <li key={link.name} className={`flex gap-3 items-center p-4 ${isActive === link.name && "bg-[#3a3a43]"}`}
                    onClick={()=>{
